...
class smaller {
    public:
    bool operator()(const typename T& a, const typename T& b) const {
        return a < b;
    }
};

//template <typename Type, typename _compare = smaller<Type>>
typedef smaller<Type> _compare;
class Tree {
   private:
    int root;
    int size;
    ...
    int insertInternal(int nodeId, Type data) {
        ...
        if (comp(data, nodes[nodeId].data))
    ...
    int findInternal(int nodeId, Type data) {
        ...
        if (data == nodes[nodeId].data)
            return nodeId;
        else if (comp(data, nodes[nodeId].data))
